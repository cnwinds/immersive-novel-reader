...
按照10个维度依次检查，记录发现的问题：
- 记录问题所在的集数和具体位置
- 描述问题内容（如"第3集温度从零下40度突然回升到零下20度"）
- 标注违反的标准（如"违反天灾进程体系一致性"）
...
```
=== 末世重生漫剧一致性检查报告 ===
检查批次：Episode-01 至 Episode-05
检查时间：[自动生成]
检查状态：【PASS / FAIL】

【维度1：剧情进度一致性】 ? PASS
- 第1-5集完成重生回忆、金手指获得、初期囤货，符合大纲设定

【维度2：天灾进程体系一致性】 ? FAIL
- 问题1：第3集写零下40度，第4集写零下35度，出现温度回升
- 问题2：大纲未提及怪物，但第5集出现了变异生物
...
2. 调用 doomsday-skill 针对性修改问题集数
3. 将修改后的内容重新提交给 doomsday-aligner 检查
4. 重复此流程直到返回 PASS 状态

**最多循环3次**：
- 如果3次修改后仍未通过，主Agent应询问用户是否接受当前版本或重新创作该批次
